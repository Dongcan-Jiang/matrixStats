...
rowCummaxs_R <- function(x) {
  mode <- storage.mode(x)
  # Change mode because a bug is detected on cummax for integer
...
for (rows in indexCases) {
  for (cols in indexCases) {
    validateIndicesTestMatrix(x, rows, cols, ftest=rowCummins, fsure=rowCummins_R, debug=TRUE)
    validateIndicesTestMatrix(x, rows, cols, ftest=function(x, rows, cols, ...) {
      t(colCummins(t(x), rows=cols, cols=rows))
    }, fsure=rowCummins_R, debug=TRUE)

    validateIndicesTestMatrix(x, rows, cols, ftest=rowCummaxs, fsure=rowCummaxs_R, debug=TRUE)
    validateIndicesTestMatrix(x, rows, cols, ftest=function(x, rows, cols, ...) {
      t(colCummaxs(t(x), rows=cols, cols=rows))
    }, fsure=rowCummaxs_R, debug=TRUE)
  }
}
