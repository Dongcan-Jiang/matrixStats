...
#define ARGUMENTS_LIST X_C_TYPE *x, R_xlen_t nrow, R_xlen_t ncol, int what, int narm, int hasna, X_C_TYPE *ans, int *is_counted, void *rows, R_xlen_t nrows, void *cols, R_xlen_t ncols
...
SEXP colRanges(SEXP x, SEXP dim, SEXP what, SEXP naRm, SEXP hasNA, SEXP rows, SEXP cols) {
  ...
  if (isReal(x)) {
    ...
    colRanges_Real[rowsType][colsType](REAL(x), nrow, ncol, what2, narm, hasna, REAL(ans), is_counted, crows, nrows, ccols, ncols);
    UNPROTECT(1);
  } else if (isInteger(x)) {
    if (what2 == 2) {
    ...
    colRanges_Integer[rowsType][colsType](INTEGER(x), nrow, ncol, what2, narm, hasna, INTEGER(ans), is_counted, crows, nrows, ccols, ncols);
