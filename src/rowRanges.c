...
SEXP rowRanges(SEXP x, SEXP dim, SEXP what, SEXP naRm, SEXP hasNA, SEXP rows, SEXP cols) {
  ...
  if (isReal(x)) {
    ...
    rowRanges_Real[rowsType][colsType](REAL(x), nrow, ncol, what2, narm, hasna, REAL(ans), is_counted, crows, nrows, ccols, ncols);
    UNPROTECT(1);
  } else if (isInteger(x)) {
    if (what2 == 2) {
    ...
    rowRanges_Integer[rowsType][colsType](INTEGER(x), nrow, ncol, what2, narm, hasna, INTEGER(ans), is_counted, crows, nrows, ccols, ncols);
